...
export class Crear_curso implements OnInit, AfterViewInit {
  ...
  validateCursoData(): boolean {
    ...
    for (const field of requiredFields) {
      if (!this.cursoData[field] || this.cursoData[field].trim() === "") {
        console.log(`El campo '${field}' está vacío o no existe.`);
        return false;
      }
    }
  ...
  formSubmit(f: NgForm) {
    ...
    if (this.cursoArchivos.iconocurso instanceof File) {
      if (
        this.validateFile(this.cursoArchivos.iconocurso, [
          "image/png",
          "image/jpeg",
        ])
      ) {
        ...
        this.showErrorAlert(
          "El icono del curso debe ser una imagen PNG o JPG."
        );
        return;
      }
    }
    ...
    return new Promise((resolve, reject) => {
      this.authService.uploadDoc(file).subscribe(
        (response: any) => {
          ...
          // Extraer el nombre del archivo del campo correcto de la respuesta
          if (response && response.file && response.file.filename) {
            // Cambia 'filename' por la clave correcta si es necesario
...
//       uploadTasks.push(
//         this.uploadFile(this.cursoArchivos.archivo_pt1, "archivo_pt1Nombre")
//       );
//     }
//     if (this.cursoArchivos.archivo_pt2 instanceof File) {
//       uploadTasks.push(
//         this.uploadFile(this.cursoArchivos.archivo_pt2, "archivo_pt2Nombre")
//       );
//     }
//     if (this.cursoArchivos.archivo_pt3 instanceof File) {
//       uploadTasks.push(
//         this.uploadFile(this.cursoArchivos.archivo_pt3, "archivo_pt3Nombre")
//       );
//     }
//     if (this.cursoArchivos.archivo_pt4 instanceof File) {
//       uploadTasks.push(
//         this.uploadFile(this.cursoArchivos.archivo_pt4, "archivo_pt4Nombre")
//       );
//     }
//     if (this.cursoArchivos.archivo_pt5 instanceof File) {
//       uploadTasks.push(
//         this.uploadFile(this.cursoArchivos.archivo_pt5, "archivo_pt5Nombre")
//       );
//     }
