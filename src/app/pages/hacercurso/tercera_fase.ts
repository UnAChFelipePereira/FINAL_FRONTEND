...
@Component({
  selector: "app-hacer-tercerafase",
...
export class HacerTercerafase implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (cursoId) {
      ...
      // Verifica si el curso ya ha sido completado
      this.authService.checkIfCourseCompleted(cursoId, this.userId, faseId).subscribe(
        (completed) => {
          this.alreadyCompleted = completed;
          if (this.alreadyCompleted) {
            this.showNextPhaseButton = true;
            // Si el curso ya está completado, deshabilita la selección de respuestas y muestra el botón "Ir a la siguiente fase"
            this.disableAnswers();
          } else {
            // Si el curso no está completado, verifica si ha sido iniciado
            this.authService.checkIfCourseStarted(cursoId, this.userId, faseId).subscribe(
              (started) => {
                if (started.started) {
              ...
              (error) => {
                console.error("Error al verificar si el curso ya fue iniciado:", error);
              }
            );
          }
        },
  ...
  submitAnswers(f: NgForm) {
    ...
    this.authService.endCourse(data).subscribe(
      (response) => {
        ...
        this.formSubmitted = true;
        this.showNextPhaseButton = true;
        this.alreadyCompleted = true; // Bloquea el formulario
      },
      (error) => {
