...
export class SettingsPageV1 implements OnInit {
  ...
  ngOnInit() {
    ...
    const lastElHeight =
      sections[sections.length - 1].getBoundingClientRect().height;
    ...
    var elmTriggerList = [].slice.call(
      document.querySelectorAll('[data-toggle="scroll-to"]')
    );
    var elmList = elmTriggerList.map(function (elm) {
      elm.onclick = function (e) {
        e.preventDefault();

        var targetAttr = elm.getAttribute("data-target")
        ...
        var targetContentPadding = targetContentStyle
          ? parseInt(targetContentStyle.getPropertyValue("padding-top"))
          : 0;

        if (targetElm) {
          var targetTop =
            targetElm.offsetTop - targetHeight - targetContentPadding;
    ...
    if (this.fotoData.fotoperfil instanceof File) {
      if (this.validateFile(this.fotoData.fotoperfil, ["image/jpeg"])) {
        ...
        this.uploadFile(this.fotoData.fotoperfil, fileName).then(
          () => {
            this.showSuccessAlert("Tu foto fue cambiada con éxito");
          },
          (error) => {
            console.error("Error al subir la imagen de perfil:", error);
            this.alertMessage =
              "Error al guardar la imagen de perfil. Inténtalo de nuevo.";
          }
        );
      } else {
        this.alertMessage =
          "Archivo en otro formato. La imagen de perfil debe ser JPG.";
      }
    } else {
        ...
        this.authService
          ...
          .subscribe(
            ...
            (error) => {
              ...
              this.showErrorAlert(
                "No se puede cambiar contraseña en este momento."
              );
            }
          );
      }
  ...
  onFileChange(event: any, field: string, fileName: string) {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (file instanceof File) {
        ...
        this.fotoData[`${field}Nombre`] = finalFileName;
      } else {
